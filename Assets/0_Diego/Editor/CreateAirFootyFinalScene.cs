...
public static class CreateAirFootyFinalScene
{
    private const string ScenePath = "Assets/0_Diego/Scenes/AirFootyFinal.unity";
    private const string TexturePath = "Assets/0_Diego/Resources/Texture/CosmicPitch.png";
    private const string ThemeFolder = "Assets/0_Diego/Resources/Theme";
    private const string MaterialFolder = "Assets/0_Diego/Resources/Material/3D";

    [MenuItem("Tools/Create Air Footy Final Scene")]
    public static void Build()
    {
        ...
        Debug.Log("Air Footy Final scene created successfully.");
    }
    ...
    private static void CreateThemedArena()
    {
        Texture2D tile = AssetDatabase.LoadAssetAtPath<Texture2D>(ThemeFolder + "/AirFootyTile.png");
        Material tileMaterial = GetTexturedMaterial("AirFootyTile3D", tile);

        GameObject floor = GameObject.CreatePrimitive(PrimitiveType.Plane);
        floor.name = "Neon Arena Floor";
        floor.transform.position = new Vector3(0f, -0.04f, 0.5f);
        floor.transform.localScale = new Vector3(2.35f, 1f, 1.3f);
        floor.GetComponent<Renderer>().sharedMaterial = tileMaterial;

        CreateBillboard("Poster One", ThemeFolder + "/AirFootyPoster1.png", new Vector3(-6f, 2.5f, 5.5f));
        CreateBillboard("Poster Two", ThemeFolder + "/AirFootyPoster2.png", new Vector3(6f, 2.5f, 5.5f));
    }

    private static void CreateBillboard(string name, string texturePath, Vector3 position)
    {
        GameObject billboard = GameObject.CreatePrimitive(PrimitiveType.Quad);
        billboard.name = name;
        billboard.transform.position = position;
        billboard.transform.rotation = Quaternion.Euler(0f, 180f, 0f);
        billboard.transform.localScale = new Vector3(3.2f, 4.1f, 1f);
        Object.DestroyImmediate(billboard.GetComponent<Collider>());
        Texture2D texture = AssetDatabase.LoadAssetAtPath<Texture2D>(texturePath);
        billboard.GetComponent<Renderer>().sharedMaterial = GetTexturedMaterial(name.Replace(" ", ""), texture);
    }
    ...
    private static void CreateMenu(Transform canvas)
    {
        ...
        CreateThemeImage("Menu Background", main.transform, ThemeFolder + "/AirFootyBackGround.png", true,
            Vector2.zero, new Vector2(1920f, 1080f));
        CreateThemeImage("Air Footy Nameplate", main.transform, ThemeFolder + "/AirFootyNameplate.png", false,
            new Vector2(0f, -125f), new Vector2(1300f, 310f));
        Button start = CreateButton("Start Button", main.transform, "START GAME", new Vector2(0f, -520f));
        Button instructionsButton = CreateButton("Instructions Button", main.transform, "INSTRUCTIONS", new Vector2(0f, -630f));

        GameObject instructions = CreatePanel("Instructions Panel", canvas);
        CreateThemeImage("Instructions Background", instructions.transform, ThemeFolder + "/AirFootyBackGround.png", true,
            Vector2.zero, new Vector2(1920f, 1080f));
        GameObject shade = CreatePanel("Reading Shade", instructions.transform);
        shade.GetComponent<Image>().color = new Color(0.005f, 0.015f, 0.06f, 0.82f);
    ...
    private static Image CreateThemeImage(string name, Transform parent, string path, bool stretch, Vector2 position, Vector2 size)
    {
        ...
        if (stretch) imageObject.transform.SetAsFirstSibling();
        return image;
    }
    ...
    private static Material GetTexturedMaterial(string name, Texture2D texture)
    {
        Material material = GetMaterial(name, Color.white);
        material.mainTexture = texture;
        EditorUtility.SetDirty(material);
        return material;
    }
