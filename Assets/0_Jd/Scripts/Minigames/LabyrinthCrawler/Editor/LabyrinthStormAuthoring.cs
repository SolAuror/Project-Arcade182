    ...
    public static class LabyrinthStormAuthoring
    {
        ...
        public static void Build()
        {
            ...
            Debug.Log(
                ...
                "Existing textures, material tuning and StormDirector settings were preserved.");
        }
        ...
        private static Texture2D BakeSkyline()
        {
            ...
            System.Random random = new System.Random(40319);

            int cursor = 0;
            while (cursor < width)
            {
                int towerWidth = random.Next(10, 34);
                int towerHeight = random.Next(18, 71);
                int right = Mathf.Min(width, cursor + towerWidth);

                for (int x = cursor; x < right; x++)
                {
